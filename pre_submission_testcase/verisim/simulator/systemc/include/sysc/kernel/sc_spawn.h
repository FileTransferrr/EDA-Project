...
class sc_spawn_base : public sc_process_host
{
public:
    virtual void semantics() = 0;

  sc_process_handle run_function(
    const char* name_p, SC_ENTRY_FUNC method_p, sc_process_host* host_p
  ) const
{
    sc_simcontext*      context_p = sc_get_curr_simcontext();
    return context_p->call_run_function_in_thread(
            name_p, method_p, host_p
        );
}
};
...
template<typename T>
class sc_spawn_object_v : public sc_spawn_base {
  public:
...
template <typename T>
inline sc_process_handle sc_run_function(
    typename T::result_type* r_p,
    T object,
    const char* name_p = 0)
{
    sc_spawn_object_v<T>* spawn_p = new sc_spawn_object_v<T>(r_p, object);
    return spawn_p->run_function(
      name_p, SC_MAKE_FUNC_PTR(sc_spawn_object_v<T>, semantics), spawn_p
    );
}
