...
void sc_dpi_task_thread_cor_fn(void*);
...
class sc_dpi_task : public sc_thread_process
{
    friend void sc_dpi_task_thread_cor_fn( void* );
public:
    sc_dpi_task( const sc_dpi_task& ) = delete;
    const sc_dpi_task& operator = ( const sc_dpi_task& ) = delete;

    sc_dpi_task(
        const char* name_p, bool free_host,
        SC_ENTRY_FUNC method_p, sc_process_host* host_p,
        const sc_spawn_options* opt_p ):
        sc_thread_process(name_p, free_host, method_p, host_p, opt_p)//, m_sv_ctx(sv_ctx)
    {
        m_process_kind = DPI_TASK_;
    }

    const char* kind() const override
        { return "sc_dpi_task_process"; }

  protected:
    void prepare_for_simulation() override;
};
